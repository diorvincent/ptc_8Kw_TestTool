...
void CExcelOut::GetColHeadIndexText(int col,char*p)
{
	if(col<26) sprintf(p,"%c",'A'+col);
	else
	{
		sprintf(p,"%c%c",'A'+col/26-1,'A'+col%26);
	}
}

void CExcelOut::SetGridValue(CPoint p,CString value)
{
/*	char x[100];GetColHeadIndexText(p.x,x);
	CString grid;grid.Format("%s%d",x,p.y+1);
	SetGridValue(grid,value);*/
	m_Range.SetItem(_variant_t((long)(p.y+1)),_variant_t((long)(p.x+1)),_variant_t(value));
}

void CExcelOut::SetGridValue(CString grid,CString value)
{
	Range rgMyRge;
	rgMyRge.AttachDispatch(m_Sheet.GetRange(_variant_t(grid),vtMissing),TRUE);
	rgMyRge.SetNumberFormatLocal(_variant_t("@"));
	rgMyRge.SetValue(_variant_t(value));
	rgMyRge.ReleaseDispatch(); 
}

CString CExcelOut::VariantToString( const VARIANT& var )
{
	USES_CONVERSION;
	CString strResult;
...
CString CExcelOut::GetGridValue(CPoint p)
{
	char x[100];GetColHeadIndexText(p.x,x);
	CString grid;grid.Format("%s%d",x,p.y+1);
	return GetGridValue(grid);
}

CString CExcelOut::GetGridValue(CString grid)
{
	Range rgMyRge;
	rgMyRge.AttachDispatch(m_Sheet.GetRange(_variant_t(grid),vtMissing),TRUE);
	VARIANT value=rgMyRge.GetValue();
	rgMyRge.ReleaseDispatch(); 
	return VariantToString(value);
}

void CExcelOut::SetFormat(CRect r,UINT uFormat)
{
	char x[100];GetColHeadIndexText(r.left,x);
	char x1[100];GetColHeadIndexText(r.right,x1);
	CString grid;grid.Format("%s%d:%s%d",x,r.top+1,x1,r.bottom+1);
	SetFormat(grid,uFormat);
}

void CExcelOut::SetFormat(CString grid,UINT uFormat)
{
	Range rgMyRge;
	rgMyRge.AttachDispatch(m_Sheet.GetRange(_variant_t(grid),vtMissing),TRUE);
	if((uFormat&DT_CENTER)==DT_CENTER) rgMyRge.SetHorizontalAlignment(_variant_t((long)xlCenter));		  
	else if((uFormat&DT_RIGHT)==DT_RIGHT) rgMyRge.SetHorizontalAlignment(_variant_t((long)xlRight));
	else rgMyRge.SetHorizontalAlignment(_variant_t((long)xlLeft));
	if((uFormat&DT_VCENTER)==DT_VCENTER) rgMyRge.SetVerticalAlignment(_variant_t((long)xlCenter));
	else if((uFormat&DT_BOTTOM)==DT_BOTTOM) rgMyRge.SetVerticalAlignment(_variant_t((long)xlBottom));
	else rgMyRge.SetVerticalAlignment(_variant_t((long)xlTop));
    rgMyRge.ReleaseDispatch(); 
}

void CExcelOut::MergeRange(CRect r)
{
	char x[100];GetColHeadIndexText(r.left,x);
	char x1[100];GetColHeadIndexText(r.right,x1);
	CString grid;grid.Format("%s%d:%s%d",x,r.top+1,x1,r.bottom+1);
...
void CExcelOut::MergeRange(CString grid)
{
	Range rgMyRge;
	rgMyRge.AttachDispatch(m_Sheet.GetRange(_variant_t(grid),vtMissing),TRUE);
    rgMyRge.Merge(vtMissing);
    rgMyRge.ReleaseDispatch(); 
}

void CExcelOut::Print()
{
	m_Book.SetSaved(TRUE);
    m_Sheet.PrintOut(vtMissing,vtMissing,vtMissing,vtMissing,vtMissing,vtMissing,vtMissing,vtMissing);
}

void CExcelOut::Release(BOOL bSave)
{
	if(bSave) m_Book.Save();
...
void CExcelOut::SetRowHeight(int row,int h)
{
	CDC dcScreen;dcScreen.Attach(::GetDC(NULL));
  	LONG yPerInch = dcScreen.GetDeviceCaps(LOGPIXELSY);
  	if (dcScreen.m_hDC != NULL) ::ReleaseDC(NULL, dcScreen.Detach());

	CString ppp;ppp.Format("%d:%d",row+1,row+1);
	Range rgMyRge;
	rgMyRge.AttachDispatch(m_Sheet.GetRange(_variant_t(ppp),vtMissing),TRUE);
	float fSize=(float)fabs(h*1440.f/yPerInch);
	fSize = (fSize+5)/10;fSize =(fSize+1)/2;
	rgMyRge.SetRowHeight(_variant_t(fSize));
	rgMyRge.ReleaseDispatch(); 
}

void CExcelOut::SetColWidth(int col,int w)
{
	CDC dcScreen;dcScreen.Attach(::GetDC(NULL));
  	LONG xPerInch = dcScreen.GetDeviceCaps(LOGPIXELSX);
	TEXTMETRIC tm;dcScreen.GetTextMetrics(&tm);int nCharWidth = tm.tmAveCharWidth;
	if (dcScreen.m_hDC != NULL) ::ReleaseDC(NULL, dcScreen.Detach());

	char pp[10];GetColHeadIndexText(col,pp);
	CString ppp;ppp.Format("%s:%s",pp,pp);
	Range rgMyRge;
	rgMyRge.AttachDispatch(m_Sheet.GetRange(_variant_t(ppp),vtMissing),TRUE);
	float fSize=((float)w*1440.f/xPerInch);
    fSize/=nCharWidth;
    fSize = (fSize+5)/10;fSize =(fSize+1)/2;
    rgMyRge.SetColumnWidth(_variant_t(fSize));
	rgMyRge.ReleaseDispatch(); 
}

void CExcelOut::AutoFit(int col)
{
	char pp[10];GetColHeadIndexText(col,pp);
	CString ppp;ppp.Format("%s:%s",pp,pp);
	Range rgMyRge,rgMyRge1;
	rgMyRge.AttachDispatch(m_Sheet.GetRange(_variant_t(ppp),vtMissing),TRUE);
    rgMyRge1.AttachDispatch(rgMyRge.GetEntireColumn());
	rgMyRge1.AutoFit();
	rgMyRge1.ReleaseDispatch();
	rgMyRge.ReleaseDispatch(); 
}

void CExcelOut::SaveAs(CString temps)
{
	if(temps.IsEmpty())
	{
		char NcsWorkDir[400];
		GetCurrentDirectory(400,NcsWorkDir);
		static char BASED_CODE szFilter[]=_T("Excel 文件(*.xls)|*.xls||");
		CFileDialog fDlg(FALSE,_T(".xls"),temps,OFN_HIDEREADONLY,szFilter);
		if(fDlg.DoModal()==IDOK) temps=fDlg.GetPathName();
		SetCurrentDirectory(NcsWorkDir);
	}
	if(temps.IsEmpty()) return;

	m_Book.SaveAs(_variant_t(temps),vtMissing,vtMissing,vtMissing,vtMissing,vtMissing,xlShared,
			vtMissing,vtMissing,vtMissing,vtMissing);
}
		
BOOL CExcelOut::Open(CString temps,CString pass)
{
	if(temps.IsEmpty())
		m_Book.AttachDispatch(m_Books.Add(_variant_t((long)xlWorksheet)));
	else
		m_Book.AttachDispatch(m_Books.Open(temps,vtMissing,vtMissing,vtMissing,vtMissing,vtMissing,vtMissing,
			 vtMissing,vtMissing,vtMissing,vtMissing,vtMissing,vtMissing));
	m_Sheet.AttachDispatch(m_Book.GetActiveSheet(),TRUE);
	if(!pass.IsEmpty()) m_Sheet.Unprotect(_variant_t(pass));
	m_Range.AttachDispatch(m_Sheet.GetCells(),TRUE);
	m_Range.SetNumberFormatLocal(_variant_t("@"));
	return TRUE;
}

BOOL CExcelOut::Create(BOOL bVisible) 
{
	BOOL b=TRUE;
	TRY b=m_App.CreateDispatch("Excel.Application.9");
	CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
	  
		if(!b)
		{ 
			TRY b=m_App.CreateDispatch("Excel.Application.10");
			CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
				
				if(!b)
				{ 
					TRY b=m_App.CreateDispatch("Excel.Application.11");
					CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
						
						if(!b)
						{ 
							TRY b=m_App.CreateDispatch("Excel.Application.12");
							CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
								
								if(!b)
								{ 
									TRY b=m_App.CreateDispatch("Excel.Application.13");
									CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
										
										if(!b)
										{ 
											TRY b=m_App.CreateDispatch("Excel.Application.14");
											CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
												
												if(!b)
												{ 
													TRY b=m_App.CreateDispatch("Excel.Application.15");
													CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
														if(!b)
														{	
															TRY b=m_App.CreateDispatch("Excel.Application.16");
															CATCH_ALL(e)  b=FALSE; END_CATCH_ALL
																if(!b)
																{
																	AfxMessageBox("无法打开Excel!");
																	return FALSE;
																}
														}
												}
...
extern "C" void APIENTRY ExcelOutList(CListCtrl*pList,CString strTitle,BOOL bPrint)
{
	CExcelOut e;
	e.Create(FALSE);
	if(e.Open(""))
	{
		CStringArray ar;int i=0;
		CDWordArray war;
		for(;;)
		{
			LVCOLUMN col;char name[100];
			col.mask=LVCF_TEXT;col.pszText=name;col.cchTextMax=100;
			if(!pList->GetColumn(i++,&col)) break;
			ar.Add(name);
		}
		e.MergeRange(CRect(1,1,ar.GetSize(),1));
		e.SetGridValue(CPoint(1,1),strTitle);
		
		int row=3;
		if(ar.GetSize()>1)
		{
			for(i=0;i<ar.GetSize();i++)
				e.SetGridValue(CPoint(1+i,row),ar.GetAt(i));
			row++;
		}
		for(i=0;i<ar.GetSize();i++)
			war.Add(ar.GetAt(i).GetLength());
		
		for(int j=0;j<pList->GetItemCount();j++)
		{
			for(i=0;i<ar.GetSize();i++)
			{
				CString s=pList->GetItemText(j,i);
				e.SetGridValue(CPoint(1+i,row+j),s);
				war.SetAt(i,max(s.GetLength(),(int)war.GetAt(i)));
			}
		}
		e.SetFormat(CRect(1,1,ar.GetSize(),pList->GetItemCount()+row-1),DT_CENTER|DT_VCENTER);
		for(i=0;i<ar.GetSize();i++)
		{
			int ww=war.GetAt(i)*12;
			if(ww>1024) ww=1024;
			e.SetColWidth(i+1,ww);
		}
		ar.RemoveAll();war.RemoveAll();
		if(bPrint)
			e.Print();
		else 
			e.SaveAs("");
