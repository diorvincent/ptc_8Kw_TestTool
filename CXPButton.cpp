// CXPButton.cpp : implementation file
//
...
CCXPButton::CCXPButton()
{
	m_BoundryPen.CreatePen(PS_INSIDEFRAME | PS_SOLID, 1, RGB(0, 0, 0));
	//橘色变更成绿色
	m_InsideBoundryPenLeft.CreatePen(PS_INSIDEFRAME | PS_SOLID, 3, RGB(37, 212, 65) /*RGB(250, 196, 88)*/);
	m_InsideBoundryPenRight.CreatePen(PS_INSIDEFRAME | PS_SOLID, 3, RGB(26, 212, 33)/*RGB(251, 202, 106)*/);
	m_InsideBoundryPenTop.CreatePen(PS_INSIDEFRAME | PS_SOLID, 2, RGB(65, 181, 108)/*RGB(252, 210, 121)*/);
	m_InsideBoundryPenBottom.CreatePen(PS_INSIDEFRAME | PS_SOLID, 2, RGB(71, 199, 118)/*RGB(229, 151, 0)*/);
...
BEGIN_MESSAGE_MAP(CCXPButton, CButton)
	//{{AFX_MSG_MAP(CCXPButton)
	ON_WM_MOUSEMOVE()
...
/////////////////////////////////////////////////////////////////////////////
// CCXPButton message handlers

//添加Owner Draw属性
void CCXPButton::PreSubclassWindow() 
{
...
void CCXPButton::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (!m_bTracking)
	{
		TRACKMOUSEEVENT tme;
...
LRESULT CCXPButton::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	m_bOver = FALSE;
	m_bTracking = FALSE;
...
LRESULT CCXPButton::OnMouseHover(WPARAM wParam, LPARAM lParam)
{
	m_bOver = TRUE;
	InvalidateRect(NULL);
	return 0;
...
void CCXPButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
//绘制按钮的底色
void CCXPButton::DoGradientFill(CDC *pDC, CRect* rect)
{
	CBrush brBk[64];
...
//绘制按钮的内边框
void CCXPButton::DrawInsideBorder(CDC *pDC, CRect* rect)
{
