...
int count = 0;
bool CSpreadSheet::createTable()
{
	count++;
	if (m_bAppend)
	{
		// Create new sheet
		m_stempSql.Format("CREATE TABLE [%s%d] (", m_sSheetName, count);
		int a=m_aFieldNames.GetSize();
...
bool CSpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		if (m_bAppend)
		{
			...
			// Create new sheet
			m_stempSql.Format("CREATE TABLE [%s%d$A1:IV65536] (", m_sSheetName, count);
		...
		for (int k = 1; k < m_dTotalRows; k++)
		{
			...
			// Create Insert SQL
			m_stempSql.Format("INSERT INTO [%s%d$A1:IV%d] (", m_sSheetName, count, k);
			for (int i = 0; i < m_atempArray.GetSize(); i++)
			{
