...
void CACPSDlg::ReadRange(int cfgnum)
{
	...
	if (!finder.FindFile(sFile))
	{
		...
		CString duty_cycle1, per50_wait_time, duty_cycle2, per100_wait_time, duty_cycle3, per90_wait_time;
		...
		duty_cycle1.Format("%1.2f", 0.20f);
		per50_wait_time.Format("%7d", 40);
		duty_cycle2.Format("%1.2f", 1.00f);
		per100_wait_time.Format("%7d", 60);
		duty_cycle2.Format("%1.2f", 0.90f);
		per90_wait_time.Format("%7d", 100);
		...
		//占空比请求等待时间和值
		::WritePrivateProfileString("RANGE", "DUTY_CYCLE_1_VALUE", duty_cycle1, sFile);
		::WritePrivateProfileString("RANGE", "PER50_WAIT_TIME", per50_wait_time, sFile);
		::WritePrivateProfileString("RANGE", "DUTY_CYCLE_2_VALUE", duty_cycle2, sFile);
		::WritePrivateProfileString("RANGE", "PER100_WAIT_TIME", per100_wait_time, sFile);
		::WritePrivateProfileString("RANGE", "DUTY_CYCLE_3_VALUE", duty_cycle3, sFile);
		::WritePrivateProfileString("RANGE", "PER90_WAIT_TIME", per90_wait_time, sFile);
		...
		cfg[cfgnum].duty_cycle_value1 = 0.20f;
		cfg[cfgnum].per50_wait_time = 40;
		cfg[cfgnum].duty_cycle_value2 = 1.00f;
		cfg[cfgnum].per100_wait_time = 60;
		cfg[cfgnum].duty_cycle_value3 = 0.90f;
		cfg[cfgnum].per90_wait_time = 60;
	...
	else
	{
		...
		//占空比请求时间和值
		GetPrivateProfileString(_T(temp), _T("DUTY_CYCLE_1_VALUE"), _T("0.20"), ch, 100, sFile);
		strtemp = ch;
		cfg[cfgnum].duty_cycle_value1 = atof(strtemp.GetBuffer(strtemp.GetLength()));
		GetPrivateProfileString(_T(temp), _T("PER50_WAIT_TIME"), _T("40"), ch, 100, sFile);
		strtemp = ch;
		cfg[cfgnum].per50_wait_time = atoi(strtemp.GetBuffer(strtemp.GetLength()));

		GetPrivateProfileString(_T(temp), _T("DUTY_CYCLE_2_VALUE"), _T("1.00"), ch, 100, sFile);
		strtemp = ch;
		cfg[cfgnum].duty_cycle_value2 = atof(strtemp.GetBuffer(strtemp.GetLength()));
		GetPrivateProfileString(_T(temp), _T("PER100_WAIT_TIME"), _T("60"), ch, 100, sFile);
		strtemp = ch;
		cfg[cfgnum].per100_wait_time = atoi(strtemp.GetBuffer(strtemp.GetLength()));

		GetPrivateProfileString(_T(temp), _T("DUTY_CYCLE_3_VALUE"), _T("0.90"), ch, 100, sFile);
		strtemp = ch;
		cfg[cfgnum].duty_cycle_value3 = atof(strtemp.GetBuffer(strtemp.GetLength()));
		GetPrivateProfileString(_T(temp), _T("PER90_WAIT_TIME"), _T("60"), ch, 100, sFile);
		strtemp = ch;
		cfg[cfgnum].per90_wait_time = atoi(strtemp.GetBuffer(strtemp.GetLength()));
...
{
	if (m_ulTicktCount1 > cfg[0].msg_lost_monitor_cycle * 1000 * 100)	//1000:报文周期		10:接收报文线程周期
	{
		m_bLostPtc1 = true;
...
bool CACPSDlg::WriteMarkByte(int nWaitTimes)
{
	...
	for (int k = 0; k < 2; k++)
	{
		...
		if (WaitForSingleObject(g_DiagRespSignaled, nWaitTimes) == WAIT_OBJECT_0)
		{
...
bool CACPSDlg::YellowLed_On(CACPSDlg* pDlg)
{
	...
	{
		pDlg->WriteErrMsg("黄灯未点亮,流程退出", pDlg);
		bRight = false;
	}
	return bRight;
}
...
UINT PreThread(LPVOID param)
{
	...
	//CAN/LIN通信(报文周期检查)
	pDlg->m_bErrMsg &= pDlg->Check_MsgCycle();
	...
	// 
	//高压电压诊断
	bStepTestResult &= pDlg->HighVol_Diagnostic();
	if (!bStepTestResult)
	{
		if (pDlg->m_bLostPtc1 || pDlg->m_bDevLost1 ||
			pDlg->m_bDevLost2 || pDlg->m_bLowPowerLost)
...
bool CACPSDlg::Invoke_WriteMarkByte()
{
	...
	{
		if (!WriteMarkByte(5000))
		{
			if (m_bEEPWrote == false)
			{
...
bool CACPSDlg::Cali_PowerSlope(int* nPTC)
{
	...
	double fKu = 0.0f, fKi = 0.0f, fKu16_4 = 0.0f, fKi16_4 = 0.0f, fKu16_5 = 0.2f, fKu16_7 = 1.0f, fKu16_8 = 0.9f;
	double dError50_1, dError100_1; 
		...
		if (CaliPowerReq(MarkByte, 0, 1000))	//2
		{
			...
			if (CaliPowerReq(MarkByte, 0, 1000))  //3
			{
				...
				if(bStep1)
				{
					bStep1 = false;
					bStep2 = false;
					//5、	通过31服务设定占空比为50%		//8Kw 使用20%
					fKu16_5 = cfg[0].duty_cycle_value1;
					nRaw = fp.makeRaw(fKu16_5, 15);
					memset(pKu, 0, sizeof(pKu));
					methord::u16toPu8(nRaw, pKu);
					strInfo.Format("设定占空比 %2.1f \\%%\\", fKu16_5 * 100);
					ShowStepMessage(strInfo, 0, 0.0f, pKu);
					...
					if (CaliPowerReq(MarkByte, 0, 1000))	//5
					{
						...
						if (bGetRightResp)	//6
						{
							...
							fKu16_7 = cfg[0].duty_cycle_value2;
							nRaw = fp.makeRaw(fKu16_7, 15);
							memset(pKu, 0, sizeof(pKu));
							methord::u16toPu8(nRaw, pKu);
							strInfo.Format("设定占空比 %2.1f \\%%\\", fKu16_7 * 100);
							ShowStepMessage(strInfo, 0, 0.0f, pKu);
								...
								if (CaliPowerReq(MarkByte, 0, 1000))
								{
									...
									if (CaliPowerReq(MarkByte, 0, 1000))				//9
									{
										...
										if (CaliPowerReq(MarkByte, 0, 1000))	//11
										{
											...
											//13、上位机通过31服务设定占空比为90%
											fKu16_8 = cfg[0].duty_cycle_value3;
											nRaw = fp.makeRaw(fKu16_8, 15);
											memset(pKu, 0, sizeof(pKu));
											methord::u16toPu8(nRaw, pKu);
											strInfo.Format("设定占空比 %2.1f \\%%\\", fKu16_8 * 100);
											ShowStepMessage(strInfo, 0, 0.0f, pKu);
											...
											if (CaliPowerReq(MarkByte, 0, 1000))	//13
											{
												...
												Sleep(cfg[0].per90_wait_time * 1000);
												KillTimer(2);
												Sleep(20);
...
bool CACPSDlg::Run_Selected_PowerMode(CACPSDlg* pDlg)
{
	...
	else //LIN请求功率测试
	{
		...
		if (pcba.high_power < cfg[g_cfgindex].power_range_low || pcba.high_power > cfg[g_cfgindex].power_range_high)
		{
			...
			pDlg->WriteErrMsg("功率控制模式故障", pDlg, 13, 1);
		}
		else
		{
			pcba.PowerMode_power = pcba.high_power;
...
bool CACPSDlg::HighVol_Diagnostic()
{
	...
	if (!bRight)
	{
		...
		WriteErrMsg("chn1 PTC高压测试失败", this, 17, 1);
	}
